...
import {
  ...
  parseLengthValue,
  sanitizeLengthInput,
  type LengthDisplayParts
} from '../lib/length';
import type { LengthState } from '../types';
import { ToolSummaryRow } from './ToolSummaryRow';

interface LengthConverterProps {
  state: LengthState;
  onStateChange: (nextState: LengthState) => void;
  onReset: () => void;
}
...
export function LengthConverter({ state, onStateChange, onReset }: LengthConverterProps) {
  ...
  const parsedValue = useMemo(() => parseLengthValue(state.inputValue), [state.inputValue]);
  const activeUnit = useMemo(() => getLengthUnit(state.inputUnit), [state.inputUnit]);
  const formattedInputValue = useMemo(() => formatLengthInput(state.inputValue), [state.inputValue]);
  const inputValueClass = useMemo(() => getInputValueClass(formattedInputValue), [formattedInputValue]);

  const groupedResults = useMemo(() => {
    const convertGroup = (units: typeof LENGTH_GROUPS.metric) =>
      units
        .filter((unit) => unit.key !== state.inputUnit)
        .map((unit) => {
          const value = parsedValue === null ? null : convertLength(parsedValue, state.inputUnit, unit.key);
          return {
            ...unit,
            value,
            display: formatLengthParts(value)
          };
        });

    return {
      metric: convertGroup(LENGTH_GROUPS.metric),
      imperial: convertGroup(LENGTH_GROUPS.imperial),
      traditional: convertGroup(LENGTH_GROUPS.traditional)
    };
  ...
  const inputUnitGroups = useMemo(
    () =>
      INPUT_UNIT_GROUPS.map((group) => ({
        ...group,
        units: LENGTH_INPUT_UNITS.filter((unit) => unit.system === group.key)
      })),
    []
  );
  ...
  const handleInputChange = (nextValue: string) => {
    onStateChange({
      ...state,
      inputValue: sanitizeLengthInput(nextValue)
    });
  };
      ...
      <div className="length-grid">
        <section className="panel length-input-panel">
          ...
          <div className="length-input-block">
            <label className="length-section-label" htmlFor="length-value-input">
              数値入力
            </label>
            ...
            <div className="length-group">
              ...
              <div className="length-result-grid imperial">
                {groupedResults.imperial.map((unit) => (
                  <LengthResultTile key={unit.key} title={unit.displayLabel} display={unit.display} />
                ))}
              </div>
            </div>
          </section>
          ...
          <section className="panel length-result-panel">
            ...
            <div className="length-result-grid traditional">
              {groupedResults.traditional.map((unit) => (
                <LengthResultTile key={unit.key} title={unit.displayLabel} display={unit.display} variant="traditional" />
              ))}
            </div>
          </section>
        </div>
